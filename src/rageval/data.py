...
UKP-DARMSTADT mirror. BioASQ uses a three-source fallback chain because
the official corpus is gated.
"""
...
def load_beir(name, split="test"):
    """Download (if needed) and load a BEIR dataset split."""
...
# first (if pre-downloaded), then the HF BeIR mirror, then mini-BioASQ.
# Mini-BioASQ has no distractors -- this is the degenerate-corpus case
...
def _bioasq_hf():
    ...
    terms, so this almost always fails now. We try a couple of alternative
    repos in case any of them is still up; the chain falls through to
    mini-BioASQ otherwise.
    """
    ...
    if last_err is not None:
        raise last_err
    raise RuntimeError("no HF BioASQ mirror responded")


def _bioasq_mini():
    """Public mini-BioASQ. Only ~28K passages; the qrel union exhausts the
    pool, so the resulting subset has zero distractors."""
    from datasets import load_dataset
...
def load_bioasq():
    ...
    corpus is small enough that the qrel union exhausts it -- the resulting
    subset has zero distractors and BM25 is inflated by roughly +0.30
    nDCG@10. See main.tex Section 4.2 for the caveat.
    ...
    """
    sources = [
        ("local BEIR directory", _bioasq_local),
    ...
    raise RuntimeError(
        ...
        "place the BEIR-format release in notebooks/datasets/bioasq/."
    )
