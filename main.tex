...
(nDCG@10 = 0.3388), while E5-small wins at nDCG@1 (0.4231). MedCPT
...
datasets, but the in-domain MedCPT cross-encoder improves NFCorpus by
0.027 and BioASQ-subset by 0.051. A learned per-query router with six
features beats Always-BM25 but does not beat a static BM25+BGE-small
hybrid (0.3434). On PubMedQA-labeled, retrieval lifts flan-t5-base
from 0.379 to 0.561. The findings support No Free Lunch: no single
retriever or reranker dominates, and biomedical gains come from
domain-aligned reranking rather than stronger general-domain models.
\end{abstract}
...
\begin{table}[ht]
    ...
    skipped due to the available compute budget.}
    ...
    \begin{tabular}{lSSSSSS}
        ...
        BioASQ-subset & \textbf{0.7544} & 0.5392 & 0.6836 & 0.6658 & 0.6733 & 0.6101 \\
...
the same effect. The BioASQ-subset row is therefore best read as a
\emph{control} on degenerate-corpus behaviour, not as evidence that
PubMed-aligned encoders are weaker than BM25 on PubMed-derived data.
...
\begin{table}[ht]
    ...
    \begin{tabular}{llSSS}
        ...
        NFCorpus      & BM25 + BGE-reranker@100      & 0.3076 & 0.3082          & +0.0006 \\
        BioASQ-subset & BM25 + BGE-reranker@100      & 0.7544 & 0.7080          & -0.0464 \\
        ...
        BioASQ-subset & BM25 + MedCPT-CE@100      & 0.7544 & \textbf{0.8057} & +0.0513 \\
        \bottomrule
    \end{tabular}
\end{table}
...
$-0.0386$, BioASQ-subset BM25 by $-0.0464$, SciFact by $-0.0109$, and
ArguAna by $-0.2428$. A stronger general-domain reranker is not
automatically a stronger pipeline. The in-domain MedCPT cross-encoder
gives clean positive deltas on the two biomedical datasets: $+0.0271$
on NFCorpus and $+0.0513$ on BioASQ-subset. The two strongest
pipelines in the study are these MedCPT-CE combinations (NFCorpus
0.3664, BioASQ-subset 0.8057). The ArguAna collapse of $-0.2428$ is
the largest single negative $\Delta$ in the report. ArguAna asks for
counter-arguments, where ``relevant'' means \emph{contradicts} the
query, and an entailment-trained cross-encoder produces misleading
scores. The reranker inherits the inductive bias of its training
data.
...
\begin{table}[ht]
    ...
    \begin{tabular}{llSSc}
        ...
        \midrule
        Dense  $-$ BM25   & Paired boot.\ (test)   & 0.0221 & {[-0.0016,\, +0.0449]} & n/a      \\
        Hybrid $-$ Dense  & Point estimate (test)  & 0.0208 & {---}                  & ---      \\
        Dense  $-$ BM25   & Query-subset (3{,}237) & 0.0261 & {[+0.0041,\, +0.0471]} & 99.0\%   \\
        Hybrid $-$ Dense  & Query-subset (3{,}237) & 0.0230 & {[+0.0123,\, +0.0344]} & 100.0\%  \\
        \bottomrule
    \end{tabular}
\end{table}
...
The result is mixed. The learned routers do not beat the strongest
static baseline: LightGBM (0.3325) trails both the Static Hybrid
(0.3434) and Always BGE-small (0.3393). They do beat Always BM25
($+0.025$ for LightGBM, $+0.015$ for logistic) and Always BGE+rerank
($+0.032$ for LightGBM), so the routing signal is real, but smaller
than the gain from picking the right static fusion. The oracle reaches
0.4097, which is $+0.0663$ above Static Hybrid: there is real per-query
heterogeneity to exploit, equivalent to a 19\% relative lift if a
perfect router existed. Closing that gap is bounded by how much of
$\Delta(q)$ the six features predict, which $R^2 = 0.071$ already
shows is small. Document-side features (corpus coverage of the topic,
qrel density) are not available at query time and are the natural next
step.
...
\begin{table}[ht]
    ...
    \begin{tabular}{lrrrl}
        ...
        Dense (MiniLM)    & 11.6 & 8.6 & 116            & 5\,MB (3633 $\times$ 384 f32) \\
        BGE-small         & 32.2 & 0.7 & 1{,}389        & 5\,MB (3633 $\times$ 384 f32) \\
        E5-small          & 33.1 & 0.6 & 1{,}561        & 5\,MB (3633 $\times$ 384 f32) \\
        SPLADE            & 38.4 & 2.8 & 353            & sparse CSR, $\approx 151$ nnz/doc \\
        MedCPT            & 51.4 & 1.2 & 867            & 11\,MB (3633 $\times$ 768 f32) \\
        \bottomrule
    \end{tabular}
\end{table}
...
High IDF / technical  & 108 & \textbf{0.2503} & 0.2301 & 0.2786 & -0.0202 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
    ...
    \begin{tabular}{lllS}
        ...
        The model is gated and the Colab session did not}} \\
        \multicolumn{4}{l}{\emph{include a successful HuggingFace
        login, so the Llama generator silently fell back to}} \\
        \multicolumn{4}{l}{\emph{flan-t5-base only.}} \\
        \multicolumn{4}{l}{\emph{BioASQ Y/N rows: not run. The
        available BioASQ loader returned no yes/no records on}} \\
        \multicolumn{4}{l}{\emph{this run, so the task was skipped at
        the dataset stage.}} \\
        \bottomrule
    \end{tabular}
\end{table}

The four populated cells produce two observations. First, retrieval
matters: closed-book accuracy is 0.379 (chance is $\approx 0.34$ for a
3-way classification), and adding any of the three retrievers raises
it to 0.561--0.563, a $+0.18$ absolute lift. Second, the choice of
retriever does not move the needle on PubMedQA + flan-t5: BM25
(0.561), BGE-small (0.563) and BGE-small + BGE-reranker (0.561) are
within $\pm 0.002$, well below $1\sigma$ of a binomial at $p=0.56$
(about 0.016). The PubMedQA-labeled corpus indexes only ten
candidate passages per question, so even a weak retriever surfaces
the relevant passage in the top five and the generator saturates. A
stronger generator might amplify smaller retriever-quality
differences, which is the natural extension once Llama runs are
available.
...
\textbf{(4) BioASQ subset is degenerate on this run.} BioASQ-subset
...
This biases the BioASQ row towards lexical retrieval; the row should
be read as a control on degenerate-corpus behaviour, not as a clean
biomedical generalisation result. Re-running with the gated official
BioASQ-BEIR release (14M documents) is the natural fix.
...
BM25 wins the no-distractor BioASQ-subset, but only because the
corpus contains no truly irrelevant documents. On the four
non-degenerate datasets BGE-small averages $+0.060$ above BM25, which
is the largest mean of any learned system, while MedCPT averages
$-0.073$ across the five datasets and collapses to 0.1332 on ArguAna.
...
gains ($+0.030$). The clean positive cases come from the in-domain
MedCPT cross-encoder ($+0.027$ on NFCorpus, $+0.051$ on
BioASQ-subset), with the opposite sign of the first-stage MedCPT
result on the same data. Domain alignment is most useful when the
model attends jointly to query and document, not when query and
document are encoded separately.

The per-query router finds real but small heterogeneity. LightGBM
beats Always-BM25 by $+0.025$ but loses to a static BM25+BGE-small
hybrid by $-0.011$. The oracle is $+0.066$ above the static hybrid,
so the routing signal exists, but the six features explain only about
7\% of $\Delta(q)$. The train--test gap follows the expected
capacity-vs-sample-size pattern: the linear router stays near the
variance baseline, the boosted router has a gap of $+0.0468$, more
than ten times the baseline, and still does not beat the static
hybrid. A capacity-controlled router is the next step.
